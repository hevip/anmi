...
class MemberUpgrade {
	...
	public function upgrade( $recommended, $fromId, $layer = 0 ) {
		...
		if ( !$this->model['integral']->addAll( $integralAll ) ) {
		...
		if ( !$this->model['upgrade']->addAll( $upgradeAll ) ) {
		...
		$saves = array();
		$saves['id'] = $member['id'];
		$saves['integral'] = $integralBM + $member['integral'];
		$saves['grown'] = $upgradeBM + $member['grown'];
		...
		// 会员升级
		$member['integral'] = $saves['integral'];
		$member['grown'] = $saves['grown'];
