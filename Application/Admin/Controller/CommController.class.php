...
class CommController extends Controller {
	...
	private function checkLogin() {
		...
		if ( empty( $userAuth ) ) {
			...
			if (empty ( $this->user )) {
				...
				// print_r(strpos( $this->user['authString'], $action ) );die;
	...
	public function getUserCompany(){
		...
		$companyList = $companyModel
					->alias('c')
					->field('c.id,c.companyname,c.logo_url,u.username,u.nickname,u.department_id,u.company_id,u.position_id')
					->join('left join db_user AS u ON u.company_id=c.id')
					->where('u.id',session('userAuth')['id'])
					->find();
		return $companyList;
	}
	/**
 ...
 */
	public function isStaffMembers(){

		$userInfo = D('User')->where(['id'=>session('userAuth')['id']])->getField('company_id');
		if ($userInfo) {
			return true;
		}else{
			return false;
