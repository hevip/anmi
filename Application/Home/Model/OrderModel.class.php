...
class OrderModel extends CommModel {
	...
	public function goBuyNowInsert($data){
		...
		$orderArray ['price'] = ($product['member_price'] * $data['num']) + ($product['freight'] * $data['num']);
		$orderArray ['is_pay'] = $orderArray ['price'] == $money ? 1 : 0; 
		
		//当$orderArray ['is_pay']== 1，订单被全额钱包支付。直接订单完成计算分销
		...
		if ( $this->create ( $orderArray ) ) {
			...
			if ( $result ) {
				...
				//钱包全额支付
				if($orderArray ['is_pay'] == 1){
					$allOrder = D( 'Order' )->getOne( $tempArray['order_id'], 'order_id' );
					$this->moneyAllPay($allOrder);
				}
				D ( 'OrderInfo' )->add ( $tempArray );
				cookie( 'TempCart', null );
				$return ['is_pay'] = $orderArray ['is_pay'];
				$return ['status'] = 1;
				$return ['info'] = $result;
			} else {
				$return ['info'] = "下单失败!";
	...
	public function insert( $data ) {
		...
		// 添加订单
		$orderArray ['create_time'] = time ();
		$orderArray ['price'] = $orderArray ['price'];
		$orderArray ['is_pay'] = $orderArray ['price'] == $money ? 1 : 0;
		if ( $this->create ( $orderArray ) ) {
			$result = $this->add ();
	...
	public function moneyAllPay($order){
		//添加钱包消费记录，减少钱包金额
