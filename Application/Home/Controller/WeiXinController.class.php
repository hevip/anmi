...
class WeiXinController extends Controller {
	...
	public function checkUser() {
		...
		// print_r($jsonObj['access_token']);die;
		...
		$memberObj = $memberModel->where( array( 'id' => $this->member['id'] ) )->find();
		// 判断微信openid是否注册
		// 	1、如果未注册，就注册、、将获取到的openID赋值给登录账号
		
		if ( empty( $this->member['wx_openid'] ) ) {

			$theArray ['wx_open_id'] = $data ['wx_open_id'];
			$theArray ['id'] = $_SESSION['member_auth']['id'];
			// $theArray ['password'] = sha1( '123456' );
			$theArray ['wx_name'] = json_encode( $data ['wx_name'] );
	// print_r($data);die;
